...
parser.add_argument("-d", "--ddir", action="store", type=str, required=False,
...
def main():
    ...
    for product in products:
        ...
        results[product] = find_missing_products(
            product=product,
            dir_download=os.path.join(args.ddir, products[product])
        )
    ...
    if input(f"Submit {s} downloads (yes/no):").lower() == "yes":
        ...
        for product in results:
            earthaccess.download(
                ...
                local_path=os.path.join(args.ddir, products[product])
            )
    else:
